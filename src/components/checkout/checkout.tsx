...
export function Checkout() {
    ...
    const config = { form: null } as any;
    config.pamentTokenCallback = (payload: any) => {
        console.log(payload)
    ...
    function next() {
        if (current === 0) {
            config.form.validateFields((err: any, values: any) => {
                if (!err) {
                    config.data = values;
...
function getContent(index: any, config: any) {
    switch (index) {
